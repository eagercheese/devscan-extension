...
try {
  ...
  console.warn(
    "[DEVScan] Failed to import url-utils.js, using embedded functions:",
    e.message
  );
}
...
class ServerConnectionManager {
  ...
  async healthCheck() {
    ...
    try {
      ...
      if (this.isHealthy) {
        ...
        console.warn(
          `[DEVScan] 💛 Server responded with status: ${response.status}`
        );
      }
  ...
  async findServer() {
    ...
    this.isHealthy = false;
    throw new Error(
      "DEVScan server not found on any port. Please ensure the server is running."
    );
  }
...
function findVerdictForUrl(url, serverVerdicts) {
  ...
  const strategies = [
    ...
    (u, verdicts) => {
      ...
      return (
        verdicts[normalized] ||
        Object.entries(verdicts).find(
          ([key]) => normalizeUrl(key) === normalized
        )?.[1]
      );
    },
    ...
    (u, verdicts) => {
      ...
      return Object.entries(verdicts).find(
        ([key]) => extractDomain(key) === domain
      )?.[1];
    },
    ...
    (u, verdicts) => {
      ...
      return (
        verdicts[baseUrl] ||
        Object.entries(verdicts).find(
          ([key]) => key.split("?")[0].split("#")[0] === baseUrl
        )?.[1]
      );
    },
  ];
  ...
  for (let i = 0; i < strategies.length; i++) {
    ...
    if (result) {
      console.log(
        `[DEVScan Background] 🎯 Found verdict using strategy ${
          i + 1
        } for ${url}`
      );
      return result;
  ...
  console.log(
    `[DEVScan Background] ❌ No verdict found for ${url} using any strategy`
  );
  return null;
}
...
class VerdictCache {
  ...
  set(
    ...
    console.log(
      `[DEVScan Background] 💾 Cached verdict for ${url}: verdict: ${verdict} | anomaly_risk_level: ${anomaly_risk_level} | confidence_score: ${confidence_score}`
    );
  }
  ...
  get(url, sessionId = null) {
    ...
    if (sessionId && this.sessionCache.has(sessionId)) {
      ...
      if (sessionEntry && this.isValid(sessionEntry)) {
        console.log(
          `[DEVScan Background] 🎯 Cache HIT (session) for ${url}: verdict: ${sessionEntry.verdict} | anomaly_risk_level: ${sessionEntry.anomaly_risk_level} | confidence_score: ${sessionEntry.confidence_score}`
        );
        return {
    ...
    if (entry && this.isValid(entry)) {
      console.log(
        `[DEVScan Background] 🎯 Cache HIT (global) for ${url}: verdict: ${entry.verdict} | anomaly_risk_level: ${entry.anomaly_risk_level} | confidence_score: ${entry.confidence_score}`
      );
      return {
  ...
  cleanup() {
    ...
    if (cleaned > 0) {
      console.log(
        `[DEVScan Background] 🧹 Cleaned ${cleaned} expired cache entries`
      );
    }
  }
  ...
  getStats() {
    return {
      ...
      totalSessionEntries: Array.from(this.sessionCache.values()).reduce(
...
class VerdictDeliveryDiagnostics {
    ...
    return {
      success: () => {
        ...
        if (deliveryTime > 1000) {
          // Log slow deliveries
          console.warn(
            `[DEVScan Background] 🐌 Slow delivery (${deliveryTime.toFixed(
              2
            )}ms): ${url}`
          );
        }
  ...
  updateAverageDeliveryTime(newTime) {
    ...
    this.stats.averageDeliveryTime =
      (this.stats.averageDeliveryTime * (totalDelivered - 1) + newTime) /
      totalDelivered;
  }
...
function sendVerdictWithAck(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (retryCount > 0) {
      logger.retry();
      console.log(
        `[DEVScan Background] � Retry ${retryCount}/${maxRetries} for ${url}`
      );
    } else {
      console.log(
        `[DEVScan Background] 📤 Sending verdict: ${url} -> ${verdict}`
      );
    }
    ...
    chrome.tabs.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          console.warn(
            `[DEVScan Background] ⚠️ Message failed (attempt ${
              retryCount + 1
            }):`,
            error.message
          );
          ...
          if (retryCount < maxRetries) {
            ...
            console.error(
              `[DEVScan Background] ❌ Max retries exceeded for ${url}`
            );
          ...
          console.log(
            `[DEVScan Background] ✅ Verdict delivered: ${url} -> ${verdict}`
          );
          logger.success();
          resolve(response);
        } else {
          if (response && response.message === "Failed to update tooltip") {
            console.log(
              `[DEVScan Background] ℹ️ No link in DOM for ${url}, skipping tooltip update.`
            );
            logger.success(); // treat as a "handled" case, not failure
            resolve(response);
          } else {
            const error = new Error(
              `Message not acknowledged: ${JSON.stringify(response)}`
            );
            console.warn(
              `[DEVScan Background] ⚠️ Message not acknowledged for ${url}:`,
              response
            );
...
function convertMLVerdictToString(verdict) {
  ...
  if (!verdict || typeof verdict !== "object") {
    console.log(
      `[DEVScan Background] 🔧 DEBUG: Invalid verdict, returning scan_failed`
    );
    return "scan_failed";
  }
  ...
  console.log(
    `[DEVScan Background] 🔧 DEBUG: final_verdict field:`,
    finalVerdict
  );

  if (
    finalVerdict.toLowerCase().includes("malicious") ||
  ...
  if (
    finalVerdict.toLowerCase().includes("anomalous") ||
  ...
  if (
    finalVerdict.toLowerCase().includes("unknown") ||
  ...
  // Default fallback
  console.log(
    `[DEVScan Background] 🔧 DEBUG: No match found, defaulting to scan_failed`
  );
  return "scan_failed";
}
...
function isSameDomain(url1, url2) {
  try {
    ...
    console.warn(
      `[DEVScan Background] Error parsing URLs for same-domain check: ${error.message}`
    );
    return false;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "openWarningTab" && message.targetUrl) {
    ...
    chrome.storage.sync.get(["strictMaliciousBlocking"], (data) => {
      ...
      const warningUrl = chrome.runtime.getURL(
        `${warningPageFile}?url=${encodeURIComponent(
          ...
          sender.tab.id
        }&riskLevel=${riskLevel}&strict=${strictBlocking}`
      );
      ...
      chrome.tabs.create(
        ...
        (tab) => {
          if (chrome.runtime.lastError) {
            console.error(
              `[DEVScan Background] Error creating tab:`,
    ...
    console.log(
      `[DEVScan Background] 🔧 DEBUG: Received analyzeSingleLink request for: ${message.url}`
    );
    console.log(`[DEVScan Background] 🔧 DEBUG: Message details:`, message);
    console.log(
      `[DEVScan Background] 🔧 DEBUG: Sender tab ID:`,
      sender.tab?.id
    );

    // Handle individual link analysis for immediate verdict delivery
    handleSingleLinkAnalysis(
      message.url,
      message.domain,
      message.sessionId,
      sender.tab.id
      ...
      if (
        ...
        console.log(
          `[DEVScan Background] Server unavailable for ${message.url}, will mark as scan failed after delay`
        );
      } else {
      ...
      setTimeout(() => {
        console.log(
          `[DEVScan Background] Sending delayed scan_failed for ${message.url} due to error after longer wait`
        );
        chrome.tabs
          ...
          .catch((msgError) => {
            console.error(
              "[DEVScan Background] Failed to send fallback verdict:",
              msgError
            );
          });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "retryScan" && message.url) {
    (async () => {
      ...
      const { currentSessionId: existingSession } =
        await chrome.storage.sync.get("currentSessionId");
      ...
      try {
        const analysisResult = await handleSingleLinkAnalysis(
          retryUrl,
          domain,
          currentSessionId,
          sender.tab.id
      ...
      if (verdict === "malicious" || verdict === "anomalous") {
        ...
        chrome.tabs.sendMessage(
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              console.error(
                "[DEVScan] Failed to send scanFailed message:",
                chrome.runtime.lastError
              );
            } else {
...
async function handleSingleLinkAnalysis(url, domain, providedSessionId, tabId) {
  try {
    ...
    try {
      ...
      console.error(
        `[DEVScan Background] ❌ Server connection failed: ${error.message}`
      );
    ...
    const { currentSessionId } = await chrome.storage.sync.get([
      "currentSessionId",
    ]);
    ...
    if (result.success && result.verdicts) {
      ...
      if (verdict) {
        ...
        console.log(
          `[DEVScan Background] 🎯 Verdict found for ${url}: ${verdictString}`
        );
        console.log(
          `[DEVScan Background] 🔧 DEBUG: Original verdict object:`,
          verdict
        );
        console.log(
          `[DEVScan Background] 🔧 DEBUG: Conversion result: "${verdict.final_verdict}" -> "${verdictString}"`
        );
        console.log(
          `[DEVScan Background] 🎯 Anomaly_risk_level result: "${verdict.anomaly_risk_level}"`
        );
        console.log(
        ...
        console.error(
          `[DEVScan Background] ❌ No verdict found for ${url} in server response`
        );
        ...
        try {
          ...
          console.error(
            `[DEVScan Background] ❌ Failed to deliver scan_failed verdict for ${url}:`,
            error
          );
        }
      }
...
async function handleExtractLinks(maliciousUrl) {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error("Extract-links request timeout")),
        8000
      );
    });
    ...
    if (!response.ok) {
      throw new Error(
        `Extract-links endpoint responded with status: ${response.status}`
      );
    }
...
async function interceptURL(url, details) {
  ...
  const decodedUrl =
    typeof decodeHexUrl !== "undefined"
      ? decodeHexUrl(details.url)
      : details.url;

  // Conditional link unshortening
  const resolvedUrl =
    typeof resolveShortenedUrl !== "undefined"
      ? await resolveShortenedUrl(decodedUrl, details)
      : decodedUrl;
  ...
  if (
    resolvedUrl.includes("html/WarningPage.html") ||
  ...
  const { currentSessionId: existingSession } = await chrome.storage.sync.get(
    "currentSessionId"
  );
  let currentSessionId = existingSession;
  if (!currentSessionId) {
...
function shouldIntercept(details) {
  try {
    ...
    // If initiator exists and is a search engine, skip too
    if (details.initiator) {
      try {
        const init = new URL(details.initiator);
        if (searchEngines.some((engine) => init.hostname.endsWith(engine)))
          return false;
      } catch {}
    }
...
console.log(
  "  - decodeHexUrl test:",
  typeof decodeHexUrl !== "undefined"
    ? decodeHexUrl(testUrl)
    : "Function not available"
);
console.log(
  "  - resolveShortenedUrl test:",
  typeof resolveShortenedUrl !== "undefined"
    ? "Function available"
    : "Function not available"
);
